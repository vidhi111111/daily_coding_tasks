# DAY 1 Task

User Registration Form

For day 1, I created a user registration form using HTML and CSS.

The main focus was to be understanding the tags, how to make form properly and to make the page responsive.

## Features I have added :

-Full Name
-Email
-Mobile Number
-DOB
-Gender(Dropdown)
-Pswd and confirm pswd
-Address
-Country and State(Dropdown)
-Submit and Reset Button
-Focus and hover effect

  ### Tech used:

  -HTML
  -CSS

### What I have practiced 

Form elements like - 'form','input','label','select','textarea' and 'button'.

In the CSS, I practiced borders,Box,Hover effect and media query to make the form responsive.

# Login Page

I also created a responsive Login Page using HTML and CSS.

## It includes:

- Email
- Password
- Remember Me
- Login button
- Forgot Password
The login form is centered on the page and adjusts according to the screen size. I also added input focus and button hover effects.

### What I Learned

This task helped me get more comfortable with HTML forms and CSS layouts. I also understood how media queries can be used to adjust a page for smaller screens instead of creating a separate layout for mobile.

The Remember Me checkbox is currently part of the UI. Its actual functionality will be added later with JavaScript.

Purpose

The main purpose of this task was to strengthen my HTML, CSS, and responsive design fundamentals.

# Day 2 Task

### Task 1:
 ## Registration Form validation
 For day 2, I added JS validation to registration form.

 ### Features I have added in it as required
 - Name valid (more than 3 char)
 - Email valid
 - M.No
 - pswd
 - Confirm pswd
 - T&C
 - Error messages
 - pswd show/hide
 - Success Message

## Tech used
 - HTML
 - CSS
 - JS

I have practiced how to validate forms, customize error messages,pswd Show/Hide and form submission handling.
Thankyou.

